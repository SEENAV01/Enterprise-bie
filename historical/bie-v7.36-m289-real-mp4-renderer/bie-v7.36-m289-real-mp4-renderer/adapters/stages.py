...
class QAStage:
    ...
    def execute(self, ctx):
        ...
        ctx.artifacts["qa_report"] = {
            "passed": not missing and not blocked and structural_ok and grounding_ok and retrieval_ok and planning_ok and script_ok and scene_ok and remotion_ok and render_ok,
            "missing":missing,
            "checks":["traceability","required-stage-artifacts","source-readiness",
                      ...
                      "remotion-project-validation","mp4-render-validation"],
                                ...
                                ("SCRIPT_VALIDATION_FAILED" if not script_ok else
                                 ("SCENE_DSL_VALIDATION_FAILED" if not scene_ok else
                                  ("REMOTION_PROJECT_VALIDATION_FAILED" if not remotion_ok else
                                   ("MP4_RENDER_FAILED" if not render_ok else None)))))))
        }
        return ctx
