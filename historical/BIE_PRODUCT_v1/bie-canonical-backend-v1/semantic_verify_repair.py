...
def compare_expected_text(expected, ocr_words):
    ...
    e=canonical_text(expected)
    d=canonical_text(detected)
    e_cmp=e.casefold()
    d_cmp=d.casefold()
    if not e:
        return {"status":"not_applicable","match":True,"expected":"","detected":d}
    return {
        "status":"matched" if e_cmp in d_cmp else "mismatch",
        "match":e_cmp in d_cmp,
        "expected":e,
        "detected":d
    }
