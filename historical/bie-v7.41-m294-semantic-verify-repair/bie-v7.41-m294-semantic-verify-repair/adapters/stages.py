...
class QAStage:
    ...
    def execute(self, ctx):
        ...
        ctx.artifacts["qa_report"] = {
            "passed": not missing and not blocked and structural_ok and grounding_ok and retrieval_ok and planning_ok and script_ok and scene_ok and remotion_ok and render_ok and visual_ok and visual_pass and readability_ok and ocr_ok and ocr_available and eq_ok and semantic_ok and semantic_valid and semantic_report.get("status") != "mismatch",
            "missing":missing,
                                     ...
                                     ("FRAME_READABILITY_FAILED" if not readability_ok else
                                      ("OCR_TEXT_BBOX_FAILED" if not ocr_ok or not ocr_available else
                                       ("EQUATION_DIAGRAM_QA_FAILED" if not eq_ok else
                                        ("SEMANTIC_VISUAL_MISMATCH" if semantic_report.get("status") == "mismatch" else None))))))))
        }
        return ctx
