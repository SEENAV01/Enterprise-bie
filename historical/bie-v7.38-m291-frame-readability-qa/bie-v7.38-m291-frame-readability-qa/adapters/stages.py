...
class QAStage:
    ...
    def execute(self, ctx):
        ...
        ctx.artifacts["qa_report"] = {
            "passed": not missing and not blocked and structural_ok and grounding_ok and retrieval_ok and planning_ok and script_ok and scene_ok and remotion_ok and render_ok and visual_ok and visual_pass and readability_ok,
            "missing":missing,
                                  ...
                                  ("REMOTION_PROJECT_VALIDATION_FAILED" if not remotion_ok else
                                   ("MP4_RENDER_FAILED" if not render_ok else
                                    ("VISUAL_QA_FAILED" if not visual_ok or not visual_pass else
                                     ("FRAME_READABILITY_FAILED" if not readability_ok else None))))))))
        }
        return ctx
