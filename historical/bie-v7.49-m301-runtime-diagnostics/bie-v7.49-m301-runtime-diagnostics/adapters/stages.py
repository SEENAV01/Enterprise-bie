...
class QAStage:
    ...
    def execute(self, ctx):
        ...
        eq_ok = ctx.artifacts.get("equation_diagram_qa",{}).get("validation",{}).get("passed",False)
        semantic_report = ctx.artifacts.get("semantic_visual_qa",{})
        semantic_ok = semantic_report.get("status") in {"verified","not_proven"}
        semantic_valid = bool(semantic_report.get("repair_patch") is not None)
        repair_artifact = ctx.artifacts.get("auto_repair",{})
        repair_ok = repair_artifact.get("status") in {"no_change_required","repair_candidate_ready"}
        ctx.artifacts["qa_report"] = {
            "passed": not missing and not blocked and structural_ok and grounding_ok and retrieval_ok and planning_ok and script_ok and scene_ok and remotion_ok and render_ok and visual_ok and visual_pass and readability_ok and ocr_ok and ocr_available and eq_ok and semantic_ok and semantic_valid and repair_ok and semantic_report.get("status") != "mismatch",
            "missing":missing,
            "checks":["traceability","required-stage-artifacts","source-readiness",
                      ...
                      "equation-diagram-validation","semantic-visual-validation",
                                   ...
                                   ("MP4_RENDER_FAILED" if not render_ok else
                                    ("VISUAL_QA_FAILED" if not visual_ok or not visual_pass else
                                     ("FRAME_READABILITY_FAILED" if not readability_ok else
                                      ("OCR_TEXT_BBOX_FAILED" if not ocr_ok or not ocr_available else
                                       ("EQUATION_DIAGRAM_QA_FAILED" if not eq_ok else
                                        ("SEMANTIC_VISUAL_MISMATCH" if semantic_report.get("status") == "mismatch" else
                                         ("AUTO_REPAIR_FAILED" if not repair_ok else None))))))))
        }
        return ctx
