...
class QAStage:
    ...
    def execute(self, ctx):
        ...
        ctx.artifacts["qa_report"] = {
            "passed": not missing and not blocked and structural_ok and grounding_ok and retrieval_ok and planning_ok and script_ok,
            "missing":missing,
            "checks":["traceability","required-stage-artifacts","source-readiness",
                      ...
                      "lesson-plan-validation","script-validation"],
                             ...
                             ("KNOWLEDGE_GROUNDING_FAILED" if not grounding_ok else
                              ("RETRIEVAL_VALIDATION_FAILED" if not retrieval_ok else
                               ("LESSON_PLAN_VALIDATION_FAILED" if not planning_ok else
                                ("SCRIPT_VALIDATION_FAILED" if not script_ok else None))))
        }
        return ctx
