...
class QAStage:
    ...
    def execute(self, ctx):
        ...
        ctx.artifacts["qa_report"] = {
            "passed": not missing and not blocked and structural_ok and grounding_ok and retrieval_ok and planning_ok and script_ok and scene_ok and remotion_ok,
            "missing":missing,
                               ...
                               ("LESSON_PLAN_VALIDATION_FAILED" if not planning_ok else
                                ("SCRIPT_VALIDATION_FAILED" if not script_ok else
                                 ("SCENE_DSL_VALIDATION_FAILED" if not scene_ok else
                                  ("REMOTION_PROJECT_VALIDATION_FAILED" if not remotion_ok else None))))))
        }
        return ctx
