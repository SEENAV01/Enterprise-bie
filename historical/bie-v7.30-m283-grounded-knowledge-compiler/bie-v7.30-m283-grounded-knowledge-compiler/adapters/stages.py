...
class PlanningStage:
    ...
    def execute(self, ctx):
        ...
        ctx.artifacts["lesson_plan"] = {
            ...
            "source_units":[u["unit_id"] for u in units]
        }
        return ctx
...
class ScriptStage:
    ...
    def execute(self, ctx):
        ctx.artifacts["script"] = {
            "sections":[
                {"id":"s1","text":"Electric charge is a property of matter.","source_blocks":["demo:p1"]},
                {"id":"s2","text":"Like charges repel and unlike charges attract.","source_blocks":[]}
            ]
        }
        return ctx
...
class SceneStage:
    ...
    def execute(self, ctx):
        ctx.artifacts["scene_dsl"] = {
            "version":"1.0",
            "scenes":[{"id":"scene-1","script":"s1"},{"id":"scene-2","script":"s2"}]
        }
        return ctx
...
class QAStage:
    ...
    def execute(self, ctx):
        ...
        ctx.artifacts["qa_report"] = {
            "passed": not missing and not blocked and structural_ok and grounding_ok,
            "missing":missing,
            "checks":["traceability","required-stage-artifacts","source-readiness",
                      "book-structure-validation","knowledge-grounding"],
            "blocked_reason":"OCR_REQUIRED" if blocked else
                             ("KNOWLEDGE_GROUNDING_FAILED" if not grounding_ok else None)
        }
        return ctx
