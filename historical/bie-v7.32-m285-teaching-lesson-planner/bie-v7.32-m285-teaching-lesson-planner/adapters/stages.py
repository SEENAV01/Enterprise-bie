...
class QAStage:
    ...
    def execute(self, ctx):
        ...
        ctx.artifacts["qa_report"] = {
            "passed": not missing and not blocked and structural_ok and grounding_ok and retrieval_ok and planning_ok,
            "missing":missing,
            ...
            "blocked_reason":"OCR_REQUIRED" if blocked else
                             ("KNOWLEDGE_GROUNDING_FAILED" if not grounding_ok else
                              ("RETRIEVAL_VALIDATION_FAILED" if not retrieval_ok else
                               ("LESSON_PLAN_VALIDATION_FAILED" if not planning_ok else None)))
        }
        return ctx
