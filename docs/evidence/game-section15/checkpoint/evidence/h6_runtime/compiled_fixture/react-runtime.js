...
export function renderGameRuntime(React, activeLevelId) {
    ...
    const nodes = level.visual_entities.map((e) => { const tag = e.state_bindings.length ? "button" : "figure"; const props = { key: e.id, "data-entity-id": e.id, "data-entity-kind": e.kind, "data-semantic-role": e.semantic_role, "data-state-bindings": e.state_bindings.join(","), "aria-label": e.accessible_label }; if (tag === "button")
        props.type = "button"; return React.createElement(tag, props, React.createElement("span", { "aria-hidden": "true" }, e.glyph + " "), React.createElement("span", { "data-entity-label": "true" }, e.accessible_label)); });
