# BIE-GAME-QA-006 — feedback correctness and no premature answer reveal

## Purpose
Implement the Section 15 GAME-internal QA capability **feedback correctness and no premature answer reveal**. This is not Section 16 QA and cannot grant product acceptance.

## Capability-specific contract
Require distinct success/retry feedback, compiled misconception feedback and explanation coverage, with no answer reveal on failure.
...
## Verification
Atomic task tests: 6. Cumulative QA checkpoint at this task: 42 tests. Batch 08 also includes 17 cross-cutting contract/pipeline/architecture tests and upstream regression gates.

## Evidence
`evidence/batch08/tasks/BIE-GAME-QA-006/TASK_RESULT.json` stores the canonical gate result, findings, metrics, QA receipt and cumulative checkpoint.
