# BIE-GAME-STRAT-001 — Revision strategy selector

## Purpose
Select or explicitly abstain using all independent strategy assessments; no keyword routing, no silent tie-breaking.

## Dependency
Batch 01 `BIE-GAME-DSL-001` studio-enterprise contracts are the typed/provenance foundation. The selector also consumes the independently verified assessments from `BIE-GAME-STRAT-002` through `BIE-GAME-STRAT-010`; therefore the internal engineering order verifies those evaluators before the selector while preserving governed task IDs.
...
## Implementation
`bie/game_engine/strategy_engine/selector.py`
