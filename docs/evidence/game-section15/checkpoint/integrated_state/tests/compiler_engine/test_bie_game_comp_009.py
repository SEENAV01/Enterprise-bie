...
from bie.game_engine.compiler_engine.adaptation_compiler import compile_adaptation as FUNC
...
class Comp009Tests(unittest.TestCase):
    def test_valid_compile(self):
        a=FUNC(compiler_context());self.assertEqual(a.kind,ArtifactKind.ADAPTATION_PROGRAM);self.assertIn('adaptation.ts',a.path);a.validate()
    ...
    def test_deterministic_priority_emitted(self):
        a=FUNC(compiler_context());self.assertIn('"deterministic_priority":true',a.content);self.assertIn('export function adapt_',a.content)
