...
from bie.game_engine.compiler_engine.state_machine import compile_state_machine as FUNC
...
class Comp004Tests(unittest.TestCase):
    def test_valid_compile(self):
        a=FUNC(compiler_context());self.assertEqual(a.kind,ArtifactKind.STATE_MACHINE);self.assertIn('state-machine.ts',a.path);a.validate()
    ...
    def test_state_schema_preserves_bounds(self):
        a=FUNC(compiler_context());self.assertIn('"min":0',a.content);self.assertIn('"max":10',a.content)
