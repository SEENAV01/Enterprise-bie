...
from bie.game_engine.compiler_engine.telemetry_compiler import compile_telemetry as FUNC
...
class Comp010Tests(unittest.TestCase):
    def test_valid_compile(self):
        a=FUNC(compiler_context());self.assertEqual(a.kind,ArtifactKind.TELEMETRY_PROGRAM);self.assertIn('telemetry.ts',a.path);a.validate()
    ...
    def test_sensitive_telemetry_rejected(self):
        c=replace(compiler_context(),telemetry_allowlist=('source_text',))
        with self.assertRaises(GameCompilerError):FUNC(c)
    def test_sanitizer_allowlist_emitted(self):
        a=FUNC(compiler_context());self.assertIn('sanitizeTelemetry',a.content);self.assertIn('raw_text_allowed',a.content)
