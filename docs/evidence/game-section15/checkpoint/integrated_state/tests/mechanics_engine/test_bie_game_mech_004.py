...
ctx=sample_context
from bie.game_engine.mechanics_engine import sequencing
def valid():return sequencing.execute(ctx(),{},('A','B','C'),{'A':(), 'B':('A',),'C':('B',)},('A','B','C'))
def invalids():return [(lambda:sequencing.execute(ctx(),{},('A','B'),{'A':('B',),'B':('A',)},('A','B'))),(lambda:sequencing.execute(ctx(),{},('A','B'),{'A':(),'B':()},('A','A')))]
from bie.game_engine.mechanics_engine import sequencing as MODULE
