...
def pool():return [{'id':'f1','prompt_ref':'prompt:1','answer_ref':'answer:1','evidence_ref':'source:1'},{'id':'f2','prompt_ref':'prompt:2','answer_ref':'answer:2','evidence_ref':'source:2'}]
def valid():return retrieval.execute(ctx(),{},pool(),['f1'],False)
def invalids():return [(lambda:retrieval.execute(ctx(),{},[],[],False)),(lambda:retrieval.execute(ctx(),{},[{'id':'x'}],[],False))]
from bie.game_engine.mechanics_engine import retrieval as MODULE
