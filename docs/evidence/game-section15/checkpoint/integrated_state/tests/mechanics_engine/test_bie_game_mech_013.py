...
ctx=sample_context
from bie.game_engine.mechanics_engine import misconception_trap
def valid():return misconception_trap.execute(ctx(),{},'wrong',{'wrong':{'correct':False,'feedback_ref':'feedback:wrong','misconception_id':'mis:1','evidence_ref':'source:1'},'right':{'correct':True,'feedback_ref':'feedback:right'}})
def invalids():return [(lambda:misconception_trap.execute(ctx(),{},'x',{})),(lambda:misconception_trap.execute(ctx(),{},'wrong',{'wrong':{'correct':False,'feedback_ref':'f','misconception_id':'m'}}))]
from bie.game_engine.mechanics_engine import misconception_trap as MODULE
