...
def compile_interactions(ctx:CompilerContext):
    ...
    for exp in ctx.document.experiences:
      for level in exp.levels:
        routes={r.action_id:r for r in compile_action_routes(level)};challenge_ids=[c.challenge_id for c in level.challenges]
        for a in level.interaction.actions:
            route=routes[a.action_id]
            rows.append({'level_id':level.level_id,'action_id':a.action_id,'kind':a.kind.value,'target':a.target_entity_id,'accessible_label':a.accessible_label,'keyboard_equivalent':a.keyboard_equivalent,'mechanic_id':route.mechanic_id,'rule_id':route.rule_id,'state_bindings':list(route.state_bindings),'route_fingerprint':route.route_fingerprint,'challenge_ids':challenge_ids})
