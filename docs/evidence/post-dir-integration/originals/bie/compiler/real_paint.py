...
def capture_entry_h8(request, measure_js, paint_js):
    source = capture_entry(request, measure_js, paint_js)
...
def produce_actual_paint(workspace,output, *, node,browser,target,policy=None):
    ...
    from .raster_capture import build_raster_targets,capture_budget,inspect_counterfactual_capture,REAL_SCOPE
    raster_targets=build_raster_targets(raw);capture_budget(target.width,target.height,n,len(raster_targets))
    ...
    with tempfile.TemporaryDirectory(prefix='bie-real-paint-') as td:
        ...
        process,kernel=run_isolated([str(node),'/engine/app/bie/compiler/qa_support/remotion_raster_capture.cjs','/work/capture-request.json'],
        ...
        records=[];paint=[]
        for f in frames:
            if f['image']!=f"frame-{f['frame']:06d}-full.png" or file_hash(capture/f['image'])[0]!=f['image_sha256']:raise CompilerQAError('ACTUAL_PAINT_IMAGE_CHANGED')
            m=f['measurement']
        ...
        raster=inspect_counterfactual_capture(capture,data.get('counterfactual'),raw,target,checked.manifest_sha256,expected_scope=REAL_SCOPE)
        ...
        evidence={'schema_version':'bie.actual-paint-verified-run.v1','source_manifest_sha256':checked.manifest_sha256,
                  ...
                  'helper_sha256':file_hash(stage/'qa-paint-helper.js')[0],'producer_sha256':file_hash(support/'remotion_raster_capture.cjs')[0],
                  'frame_count':n,'fit':fit,'paint':quality,'raster':raster,'font_coverage':font_evidence,'kernel_policy':kernel,'real_remotion':True,
        ...
        return ActualPaintWitness(checked.manifest_sha256,digest(evidence),before['identity_sha256'],n,fit['passed'] and quality['passed'] and raster['passed'],evidence,_SEAL)
