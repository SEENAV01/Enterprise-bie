#!/usr/bin/env python3
"""Explicit SOURCE-only H3 benchmark. Never a full-dependency/render acceptance run."""
...
def worker(corpus,index):
    ...
    try:
        r=compile_h3_scene(c['document'],target=CompilerQATarget(**c['target']),motion_preference=c['preference'])
...
def run(corpus,output,runs):
    ...
    for i,c in enumerate(cases):
        ...
        match=r['source_passed']==c['expected_source_pass'] and set(c['expected_codes'])<=set(r['codes'])
        rows.append({'case_id':c['case_id'],'expected_source_pass':c['expected_source_pass'],'expected_codes':c['expected_codes'],'source_expectation_matched':match,'independent_process_determinism_matched':det,'observations':obs})
    out={'schema_version':'bie.h3-source-benchmark.v1','scope':'SYNTHETIC_SOURCE_GENERATION_NOT_DEPENDENCY_COMPILE_OR_RENDER','corpus_sha256':sha256(corpus.read_bytes()).hexdigest(),'runner_sha256':sha256(Path(__file__).read_bytes()).hexdigest(),'case_count':len(cases),'independent_worker_executions':len(jobs),'runs_per_case':runs,'cases':rows,'source_expectations_matched':all(r['source_expectation_matched'] for r in rows),'determinism_matched':all(r['independent_process_determinism_matched'] for r in rows),'full_dependency_compile':'NOT_RUN_BY_THIS_SOURCE_BENCHMARK','real_remotion':'NOT_RUN','real_book':'NOT_RUN','accepted':False}
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--corpus',type=Path,default=ROOT/'fixtures/comp_h3/source_corpus.json');p.add_argument('--output',type=Path);p.add_argument('--runs',type=int,default=3);p.add_argument('--worker',type=int);a=p.parse_args()
