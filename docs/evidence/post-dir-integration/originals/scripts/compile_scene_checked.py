...
def main():
    ...
    parser.add_argument("--asset-root",type=Path,help="Local asset root: H6 narration/ and H11 assets/; actual bytes required for publication")
    ...
    try:
        ...
        if args.layout_policy is not None:
            ...
            result=repair_and_publish(json.loads(args.scene.read_text()),json.loads(args.layout_policy.read_text()),args.destination,args.layout_evidence,browser=args.browser,motion_preference=args.motion_preference,target=target,asset_root=args.asset_root)
        ...
        receipt=publish_h3_scene(json.loads(args.scene.read_text()),args.destination,motion_preference=args.motion_preference,target=target,asset_root=args.asset_root)
