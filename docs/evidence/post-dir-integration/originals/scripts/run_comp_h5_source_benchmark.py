#!/usr/bin/env python3
"""Explicit SOURCE-only H5 benchmark. Never a full-dependency/render acceptance run."""
...
def worker(corpus,index):
    ...
    try:
        r=compile_h3_scene(c['document'],target=CompilerQATarget(width=1280,height=720,fps=24,compiler_version='1.3.0-comp-h3'),motion_preference='standard')
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--corpus',type=Path,default=ROOT/'fixtures/comp_h5/corpus.json');p.add_argument('--output',type=Path);p.add_argument('--runs',type=int,default=3);p.add_argument('--worker',type=int);a=p.parse_args()
