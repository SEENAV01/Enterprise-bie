...
def diagnostic(raw,frames):
    ...
    with tempfile.TemporaryDirectory() as td:
        p=Path(td)/'request.json';p.write_text(json.dumps(request));r=__import__('subprocess').run(['node',str(ROOT/'app/bie/compiler/qa_support/media_bridge.cjs'),str(p)],capture_output=True,text=True,timeout=35)
