...
class RealProducerTests(unittest.TestCase):
    ...
    def test_actual_cjs_syntax_real_node(self):
        p=Path(__file__).parents[2]/'app/bie/compiler/qa_support/remotion_paint_capture.cjs';r=subprocess.run(['node','--check',str(p)],capture_output=True,text=True);self.assertEqual(r.returncode,0,r.stderr)
    def test_actual_entry_has_no_double_import(self):
        p=Path(__file__).parents[2]/'app/bie/compiler/qa_support/remotion_paint_capture.cjs';s=p.read_text();self.assertIn('@remotion/renderer',s);self.assertIn('@remotion/bundler',s);self.assertNotIn('layout_bridge',s);self.assertIn('renderStill',s);self.assertIn('renderMedia',s)
