# BIE-AUDIO-H1-001 — Versioned neural deployment and capability negotiation
...
Source: neural_policy.py. Complete cross-task contract and limits: `docs/audio/hardening_h1/CONTRACTS.md`.

Test suite: `tests/audio/test_audio_h1_001.py`, 22 declared tests. Use `python -B scripts/run_audio_tests.py --pattern test_audio_h1_001.py --output /tmp/audio-h1-001.json` from the extracted package. Tests use explicit offline provider fixtures; subprocess transport and native audio processing are actual local execution.
