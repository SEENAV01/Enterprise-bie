# BIE-AUDIO-H1-002 — Bounded official HTTPS and credential-safe cancellation
...
Source: neural_transport.py. Complete cross-task contract and limits: `docs/audio/hardening_h1/CONTRACTS.md`.

Test suite: `tests/audio/test_audio_h1_002.py`, 20 declared tests. Use `python -B scripts/run_audio_tests.py --pattern test_audio_h1_002.py --output /tmp/audio-h1-002.json` from the extracted package. Tests use explicit offline provider fixtures; subprocess transport and native audio processing are actual local execution.
