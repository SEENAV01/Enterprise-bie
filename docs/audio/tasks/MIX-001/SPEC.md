# BIE-AUDIO-MIX-001 — loudness normalization

Original Section 14 task, no newly invented numbering. Continuation of the supplied Batch 004 recovery source.

Implementation: `bie/audio/loudness_normalization.py, mix_meter.py, mix_contract.py`. Exact supported behavior, source inputs, limits, error conditions and exclusions: `docs/audio/BATCH004_CONTRACTS.md`, MIX-001 section.

Atomic test suite: `tests/audio/test_audio_mix_001.py`. Combined boundaries: `test_audio_batch004_integration.py`, `test_audio_mix_io.py`; native benchmark: `scripts/benchmark_audio_batch004.py`.
