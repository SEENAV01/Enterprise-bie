# BIE-AUDIO-MIX-005 — peak control

Original Section 14 task, no newly invented numbering. Continuation of the supplied Batch 004 recovery source.

Implementation: `bie/audio/peak_control.py, mix_pipeline.py`. Exact supported behavior, source inputs, limits, error conditions and exclusions: `docs/audio/BATCH004_CONTRACTS.md`, MIX-005 section.

Atomic test suite: `tests/audio/test_audio_mix_005.py`. Combined boundaries: `test_audio_batch004_integration.py`, `test_audio_mix_io.py`; native benchmark: `scripts/benchmark_audio_batch004.py`.
