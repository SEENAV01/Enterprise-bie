...
def _compile_context(io, source_ref, concepts, prerequisites, derivations, observation_refs, policy):
    ...
    observations = _observations(io, observation_refs, source_ref, concept_ids, policy)
    states = tuple(infer_knowledge_state(c, observations) for c in sorted(concept_ids))
    return {'knowledge_graph': knowledge, 'knowledge_graph_checks': kg_qa,
            'prerequisite_order': order, 'math': math, 'mastery_states': [asdict(s) for s in states],
            'mastery_interpretation': 'REPORTED_OBSERVATIONS_NOT_AUTHENTICATED_OR_CALIBRATED; NEVER_WAIVE_PREREQUISITES',
...
def publish_teaching_context(io, run_id, source_ref, concepts, prerequisites=(), derivations=(),
                             observation_refs=(), policy=TeachingContextPolicy()):
    ...
    compiled = _compile_context(io, source_ref, concepts, prerequisites, derivations, refs, policy)
    return io.derive('pedagogy.teaching_context', run_id, (source_ref,) + refs,
        {'schema_version': 'bie.dir.teaching_context/1.0.0', 'source_catalog_ref': asdict(source_ref),
         ...
         'policy': asdict(policy), 'compiled': compiled}, stage_id='PEDAGOGY',
...
def load_teaching_context(io, ref, source_ref):
    ...
    p = fields(artifact.payload, ('schema_version', 'source_catalog_ref', 'concepts', 'prerequisites',
        'derivations', 'observation_refs', 'policy', 'compiled'), 'teaching context')
    if p['schema_version'] != 'bie.dir.teaching_context/1.0.0' or reference(p['source_catalog_ref']) != source_ref:
    ...
    expected = _compile_context(io, source_ref, concepts, prerequisites, derivations, refs, decode(TeachingContextPolicy, p['policy']))
