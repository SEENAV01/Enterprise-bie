...
generator and code match. Factual QA, annotations and review execute again.
Partial-scene checkpointing and scene-local patch synthesis remain separate work.
"""
...
def prepare_repair(executor, context, inputs, previous_revision=None):
    ...
    if request is None:
        return None, ()
    fields(request, ('previous_idempotency_key',), 'director repair')
    ...
    receipt = executor.io.derive('evidence.director_repair', context.run_id, parents,
        {'schema_version': 'bie.dir.phase_repair/1.0.0', 'previous_idempotency_key': previous_key,
         ...
         'action': 'REUSE_PLAN_NARRATION_RECHECK_QA' if reused else 'REGENERATE',
         'reused_phases': ['PLAN', 'NARRATE', 'COMPILE'] if reused else [],
