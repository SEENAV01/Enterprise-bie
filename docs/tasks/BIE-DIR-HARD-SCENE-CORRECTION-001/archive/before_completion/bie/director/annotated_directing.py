...
class AnnotationRuntime:
    ...
    def enrich(self,io,inputs,base,factual_provider,factual_identity,semantic_policy):
        self.validate()
        if base.status=='BLOCKED':raise DirectingFailure('ANNOTATION_REQUIRES_NONBLOCKED_BASE',base.generation_attempts,'DIR_QA')
        try:production=produce_annotations(io,inputs,base,self.annotator,self.annotator_identity,self.policy)
        ...
        return AnnotatedDirectorResult(base,production,review,execution,semantic,qa,tuple(applied),(
            ...
            'Source/voice edits require fresh generation/review and downstream invalidation; selective repair and actual consumer/audio/game verification remain open.'))
