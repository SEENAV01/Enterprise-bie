...
def model_context(inputs):
    ...
    support = getattr(inputs, 'prerequisite_support', None)
    if support is not None:
        selected.update(d for b in support['teaching_bindings'] for d in b['reasoning_decision_ids'])
    ...
    if support is not None:
        evidence.update(e for row in support['objectives'] + support['decisions'] for e in row['evidence_ids'])
    ...
    if inputs.teaching_context is not None: evidence.update(inputs.teaching_context.evidence_ids())
    passages = tuple(p for p in inputs.catalog.passages if p.evidence_id in evidence)
    page_ids = {p.page_id for p in passages}
    result = {"input_fingerprint": inputs.fingerprint(), "lesson_id": inputs.lesson_id, "title": inputs.title,
    ...
    if inputs.teaching_context is not None: result['teaching_context'] = inputs.teaching_context.model_data()
    if support is not None: result['prerequisite_support'] = support
    return result


def validate_plan(value, inputs, policy):
    from .contextual_teaching import plan_schema, ContextPlannedScene, validate_context_plan
    ...
    for row in records:
        fields(row, plan_schema(inputs)['properties']['scenes']['items']['required'], "planned scene")
        for key in ("scene_id", "title", "purpose", "teaching_mode", "teaching_goal"): nonblank(row[key], key)
        scene = PlannedScene(**{**{k:v for k,v in row.items() if k != 'teaching_obligation_ids'}, **{key: _id_list(row[key], key, required=key not in ("parent_scene_ids", "assessment_item_ids"))
        ...
        if inputs.teaching_context is not None:
            scene = ContextPlannedScene(**asdict(scene), teaching_obligation_ids=_id_list(row['teaching_obligation_ids'], 'teaching obligation ids', False))
    ...
    validate_context_plan(inputs, scenes)
...
def validate_narration(value, inputs, plan, scene, policy):
    from .contextual_teaching import narration_schema, ContextNarratedScene, realize_context
    fields(value, narration_schema(inputs)["required"], "narrated scene")
    ...
    narration = NarratedScene(scene.scene_id, tuple(beats), tuple(assessments), _id_list(value["review_reasons"], "narration review", False))
    if inputs.teaching_context is not None:
        return ContextNarratedScene(narration.scene_id, narration.beats, narration.assessments, narration.review_reasons,
            realize_context(inputs, scene, beats, value['teaching_realizations']))
    return narration
...
def execute_grounded_director(io, inputs, generator, generator_identity, critic, critic_identity,
    ...
    from .contextual_teaching import plan_schema, narration_schema, instruction_suffix
    ...
    from .context_windows import WindowedDirectingPolicy
    if isinstance(policy, WindowedDirectingPolicy):
        from .windowed_directing import execute_windowed_director
        return execute_windowed_director(inputs, generator, generator_identity, critic, critic_identity, policy, semantic_policy)
    context = model_context(inputs)
    plan, attempts = invoke_structured(generator, generator_identity, policy, "PLAN", inputs.lesson_id,
        PLAN_INSTRUCTION + instruction_suffix(inputs), {"operation": "PLAN", "prompt_version": policy.plan_prompt_version, "inputs": context},
        plan_schema(inputs), lambda value: validate_plan(value, inputs, policy))
    narrated = []
    for scene in plan.scenes:
        try:
            content, trace = invoke_structured(generator, generator_identity, policy, "NARRATE", scene.scene_id,
                NARRATION_INSTRUCTION + instruction_suffix(inputs), {"operation": "NARRATE", "prompt_version": policy.narration_prompt_version,
                    "inputs": context, "plan": asdict(plan), "plan_fingerprint": plan.fingerprint(), "scene": asdict(scene),
                    "prior_narration": [asdict(n) for n in narrated]}, narration_schema(inputs),
        ...
        _check_narration_budget(narrated, policy, attempts)
    return _finish_director(inputs, plan, tuple(narrated), attempts, critic, critic_identity, semantic_policy)


def _check_narration_budget(narrated, policy, attempts):
...
def _finish_director(inputs, plan, narrated, attempts, critic, critic_identity, semantic_policy):
