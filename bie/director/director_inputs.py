...
class DirectorInputs:
    ...
    teaching_context: object | None = None
    ...
    def to_model_data(self):
        ...
        if self.teaching_context is None: result.pop('teaching_context')
        return result
...
def _ped_payload(payload, catalog, reasoning):
    expected = ("source_catalog_ref", "reasoning_ref", "plan", "objectives", "bindings")
    if 'teaching_context_ref' in payload: expected += ('teaching_context_ref',)
    fields(payload, expected, "pedagogy artifact payload")
...
def publish_pedagogy(io, run_id, source_ref, reasoning_ref, plan, objectives, bindings, *, teaching_context_ref=None):
    ...
    parents = (source_ref, reasoning_ref)
    if teaching_context_ref is not None:
        from .teaching_context import load_teaching_context
        load_teaching_context(io, teaching_context_ref, source_ref)
        payload['teaching_context_ref'] = asdict(teaching_context_ref)
        parents += (teaching_context_ref,); review = True
    return io.derive("pedagogy.plan", run_id, parents, payload, stage_id="PEDAGOGY", metadata={"requires_review": review})
...
def load_director_inputs(io, reasoning_ref, pedagogy_ref, *, lesson_id, title, language, run_id):
    ...
    expected_parents = {source_ref, reasoning_ref}; context = None
    if 'teaching_context_ref' in pp:
        from .teaching_context import load_teaching_context
        context_ref = reference(pp['teaching_context_ref']); expected_parents.add(context_ref)
        context = load_teaching_context(io, context_ref, source_ref)
    if set(ped_artifact.parent_refs) != expected_parents: raise ValueError("PED parent refs mismatch")
    ...
    if context is None and any(not set(d.parent_decision_ids) <= selected_ids for d in plan.decisions if d.decision_id in selected_ids):
        raise ValueError("cross-lesson prerequisite needs an explicit upstream bridge")
    lesson_context = None
    if context is not None:
        from .context_bridges import bind_lesson_context
        selected, objectives, lesson_context = bind_lesson_context(context, plan, objectives, bindings, reasoning, lesson_id)
    needed_objectives = {oid for b in selected for oid in b.objective_ids}
    review = set(source_review)
    if context is not None: review.update(context.review_reasons)
    ...
    return DirectorInputs(lesson_id, title, language, catalog, sources, reasoning, tuple(inferences), plan,
        ...
        (source_ref, reasoning_ref, pedagogy_ref), tuple(sorted(review)), lesson_context)
