...
class DirectorConsumers:
    ...
    def _director(self, ref):
        ...
        if 'base_result' in raw:
            if not {'annotation_production','annotation_review'}<=raw.keys():raise ValueError('annotated result lacks actual annotation/review evidence')
            from .windowed_annotation_review import verify_persisted_annotation_bundle
            verify_persisted_annotation_bundle(raw,inputs,base,execution,reports)
