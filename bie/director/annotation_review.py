...
def review_annotations(inputs,base,production,provider,identity,policy=AnnotationReviewPolicy()):
    ...
    if isinstance(policy,HierarchicalAnnotationReviewPolicy):
        from .hierarchical_annotation_review import review_hierarchical_annotations
        return review_hierarchical_annotations(inputs,base,production,provider,identity,policy)
    from .annotation_window_context import WindowedAnnotationReviewPolicy
    if isinstance(policy,WindowedAnnotationReviewPolicy):
        from .windowed_annotation_review import review_windowed_annotations
        return review_windowed_annotations(inputs,base,production,provider,identity,policy)
...
def approved_subjects(annotations,review):
    from .annotation_window_context import WindowedAnnotationReviewPolicy
    from .windowed_annotation_review import WindowedAnnotationReview,verify_aggregate
    if (isinstance(review,(WindowedAnnotationReview,)) or isinstance(review.policy,WindowedAnnotationReviewPolicy)
            or any(a.phase=='REVIEW_ANNOTATION_WINDOW' for a in review.attempts)):
        verify_aggregate(annotations,review)
