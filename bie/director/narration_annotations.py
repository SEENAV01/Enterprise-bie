...
def verify_base(io,inputs,base):
    ...
    from .windowed_directing import WindowedDirectorResult, verify_window_execution
    if any(a.phase in ('PLAN_WINDOW','NARRATE_WINDOW') for a in base.generation_attempts) and not isinstance(base,WindowedDirectorResult):
        raise ValueError('window generation requires its retained execution schedule and continuity evidence')
    if isinstance(base,WindowedDirectorResult): verify_window_execution(inputs,base)
    return validate_snapshot(base.execution.snapshot)
...
def validate_annotations(value,inputs,base,policy):
    ...
    if inputs.teaching_context is not None: known_concepts.update(inputs.teaching_context.selected_concept_ids)
    def rows(key):
...
def verify_production(production,inputs,base):
    ...
    if isinstance(production,WindowedAnnotationProduction) or isinstance(getattr(getattr(production,'annotations',None),'policy',None),WindowedAnnotationPolicy):
        return verify_windowed_production(production,inputs,base)
