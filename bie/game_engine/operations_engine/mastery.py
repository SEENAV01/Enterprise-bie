...
class PersistentMasteryStore:
    ...
    def update(self,learner,objective,outcome_code,mastery_weight,evidence_strength,event_id):
        ...
        require_id(objective,'GAME_OPS_OBJECTIVE');require_id(event_id,'GAME_OPS_EVENT_ID')
        success_codes={'applied','correct','success','mastered','succeeded'}
        if outcome_code not in success_codes|{'incorrect','failed','failure'}:raise GameOperationsError('GAME_OPS_TERMINAL_OUTCOME_REQUIRED')
        if any(type(x) not in (int,float) or not math.isfinite(x) or not 0<x<=1 for x in (mastery_weight,evidence_strength)):raise GameOperationsError('GAME_OPS_EVIDENCE_WEIGHT')
        fp=fingerprint({'outcome':outcome_code,'mastery_weight':mastery_weight,'evidence_strength':evidence_strength})
        with self.db:
            self.db.execute('BEGIN IMMEDIATE')
            previous=self.db.execute('SELECT input_fp,record_json FROM mastery_events WHERE learner=? AND objective=? AND event=?',(learner,objective,event_id)).fetchone()
            if previous:
                if previous[0]!=fp:raise GameOperationsError('GAME_OPS_LEARNING_REPLAY_CONFLICT')
                return MasteryRecord(**json.loads(previous[1])).validate()
            ...
            success=1.0 if outcome_code in success_codes else 0.0;weight=max(.01,min(1.0,float(mastery_weight)*float(evidence_strength)));alpha=min(.55,.12+.43*weight)
            ...
            self.db.execute('INSERT INTO mastery_state VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(learner_key_hash,objective_id) DO UPDATE SET estimate=excluded.estimate,confidence=excluded.confidence,attempts=excluded.attempts,evidence_count=excluded.evidence_count,version=excluded.version,last_event_id=excluded.last_event_id,updated=excluded.updated',(rec.learner_key_hash,rec.objective_id,rec.estimate,rec.confidence,rec.attempts,rec.evidence_count,rec.version,rec.last_event_id,time.time()))
            self.db.execute('INSERT INTO mastery_events VALUES(?,?,?,?,?)',(learner,objective,event_id,fp,json.dumps(asdict(rec),sort_keys=True)))
        return rec
