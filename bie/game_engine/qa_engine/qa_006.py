...
def evaluate(document,bundle):
    ...
    for exp in document.experiences:
      for level in exp.levels:
       for ch in level.challenges:source.append(((exp.game_id,level.level_id,ch.challenge_id),ch))
    ...
    rows={(x.get('game_id'),x.get('level_id'),x['challenge_id']):x for x in data['feedback']}
    if len(rows)!=len(data['feedback']):findings.append(finding('BIE-GAME-QA-006',7,'FEEDBACK_DUPLICATE_SCOPE','duplicate game/level/challenge identity',refs=refs))
    if set(rows)-{key for key,ch in source}:findings.append(finding('BIE-GAME-QA-006',8,'FEEDBACK_EXTRA_SCOPE','unexpected game/level/challenge identity',refs=refs))
    for key,ch in source:
        row=rows.get(key)
    ...
    cov=ratio(len(source)-sum(1 for key,ch in source if key not in rows),len(source));score=1 if not findings else max(0,cov-.15*len(findings));metrics=(QualityMetric('feedback_compile_coverage',cov,1,1),QualityMetric('blocking_feedback_findings',float(sum(f.blocking for f in findings)),0,0,'count'))
