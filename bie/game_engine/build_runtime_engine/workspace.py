...
from ..react_runtime_engine.vendor import materialize_vendor
...
def build_workspace(ctx:CompilerContext,asset_blobs,root:Path,policy=BuildPolicy()):
    ...
    (runtime/'index.html').write_text(html);asset_map={ref:'../'+rel for ref,rel,_ in asset_rows};entry=ENTRY_JS.replace('__ASSET_BINDINGS__',json.dumps(asset_map,sort_keys=True,separators=(',',':')));(runtime/'entry.js').write_text(entry)
    ...
    if ctx.experience_profile is not None:
        rights={'schema_version':'bie.game.rights/1','records':[r.__dict__ for r in ctx.experience_profile.rights],'product_accepted':False}
        (runtime/'rights-attribution.json').write_text(json.dumps(rights,sort_keys=True,separators=(',',':')))
    artifacts=[];total=0
