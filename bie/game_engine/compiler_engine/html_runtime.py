...
def compile_html_runtime(ctx:CompilerContext):
    ctx.validate();exp=ctx.document.experiences[0];level=exp.levels[0];locale='en';direction='ltr'
    if ctx.experience_profile is not None: locale=ctx.experience_profile.catalog().locale;direction=ctx.experience_profile.catalog().direction
    html='<!doctype html><html lang="'+escape(locale)+'" dir="'+escape(direction)+'"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="'+escape(csp_value(),quote=True)+'"><title>'+escape(exp.title)+'</title></head><body><main id="bie-game-root" role="application" aria-label="'+escape(exp.title)+'" data-studio-grade="true" data-slide-deck="false" data-level-id="'+escape(level.level_id)+'"></main><script type="module" src="./bootstrap.js"></script></body></html>'
