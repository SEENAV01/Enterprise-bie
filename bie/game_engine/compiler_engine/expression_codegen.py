...
def compile_expr(expr:Expr,variables=None)->str:
    ...
    if isinstance(expr,Literal):
        if type(expr.value) is int and abs(expr.value)>2**53-1:raise GameCompilerError('GAME_COMP_EXPR_INTEGER_PRECISION')
    ...
    if isinstance(expr,Compare):
        ...
        if expr.op not in (CompareOp.EQ,CompareOp.NE) and 'unknown' not in (a,b) and (a!=b or a not in ('number','string')):
            raise GameCompilerError('GAME_COMP_EXPR_COMPARABLE_REQUIRED')
    ...
    if isinstance(expr,Boolean):
        if any(_kind(x,variables or {}) not in ('boolean','unknown') for x in expr.values):raise GameCompilerError('GAME_COMP_EXPR_BOOL_REQUIRED')
    ...
    if isinstance(expr,Not):
        if _kind(expr.value,variables or {}) not in ('boolean','unknown'):raise GameCompilerError('GAME_COMP_EXPR_BOOL_REQUIRED')
        return '(!'+compile_expr(expr.value,variables)+')'
