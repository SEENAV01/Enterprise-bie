...
def diagnostic_message(error: object) -> str:
    """Represent external diagnostic controls literally; keep strict finding tokens.

    Backends may emit multiline errors. Escaping them at the receipt boundary
    preserves the reason while preventing malformed logs or a QA-path crash.
    """
    message = str(error) or type(error).__name__
    return "".join(f"\\u{ord(c):04x}" if ord(c) < 32 or ord(c) == 127 else c
                   for c in message)
