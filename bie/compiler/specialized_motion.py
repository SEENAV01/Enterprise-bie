...
def specialized_contract(track):
    ...
    discrete = p.get('_bie_h7_discrete_progress')
    if discrete is not None:
        if not isinstance(discrete, Mapping) or set(discrete)!={'fps','milestones'}:
            fail('H7_VARIANT_INTERNAL_INVALID','invalid derived progress')
        p = {k:v for k,v in p.items() if k!='_bie_h7_discrete_progress'}
    ...
    if action == 'camera':
        ...
        _fields(p, {'schema_version', 'mode', 'states', 'transition_fraction'}, {'easing','glyph_pairs'})
        if p['mode'] not in {'typeset-state-crossfade','glyph-matched-affine'}:
            fail('EQUATION_MORPH_MODE_UNSUPPORTED','explicit supported typeset/glyph contract required')
        if (p['mode']=='glyph-matched-affine') != ('glyph_pairs' in p):
            fail('GLYPH_PAIR_COVERAGE','glyph correspondence required only in explicit glyph mode')
        ...
        if p['mode']=='glyph-matched-affine':
            pairs=p['glyph_pairs']
            if not isinstance(pairs,(list,tuple)) or len(pairs)!=len(checked)-1:
                fail('GLYPH_PAIR_COVERAGE','one correspondence per transition')
            if any(not isinstance(rows,(list,tuple)) or not 1<=len(rows)<=1024 or any(not isinstance(row,(list,tuple)) or len(row)!=2 or any(type(v)is not int or v<0 for v in row) for row in rows) for rows in pairs):
                fail('GLYPH_PAIR_INVALID','bounded two-integer pairs required')
            parameters['glyph_pairs']=[[list(row) for row in rows] for rows in pairs]
        owned = ('content',)
    else:
        ...
        owned = ('content',)
    if discrete is not None:
        ms=discrete['milestones']
        if type(discrete['fps']) is not int or not 1<=discrete['fps']<=240 or not isinstance(ms,(tuple,list)) or not 2<=len(ms)<=32:
            fail('H7_VARIANT_INTERNAL_INVALID','fps/milestone contract invalid')
        for row in ms:
            _fields(row,{'frame','progress','observation_ref'})
            if type(row['frame']) is not int or row['frame']<0:fail('H7_VARIANT_INTERNAL_INVALID','frame invalid')
            _number(row['progress'],'progress',0,1);_text(row['observation_ref'],'observation_ref',512)
        if any(b['frame']<=a['frame'] or b['progress']<=a['progress'] for a,b in zip(ms,ms[1:])):
            fail('H7_VARIANT_INTERNAL_INVALID','order invalid')
        parameters['_bie_h7_discrete_progress']={'fps':discrete['fps'],'milestones':[dict(m) for m in ms]}
...
def progress_at(contract, frame, fps):
    ...
    discrete=contract.parameters.get('_bie_h7_discrete_progress')
    if discrete is not None:
        if fps!=discrete['fps']:fail('H7_VARIANT_FPS','reduced variant target changed')
        return next((m['progress'] for m in reversed(discrete['milestones']) if frame>=m['frame']),0.)
