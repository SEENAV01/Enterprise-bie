...
def _run_candidates(payload, policy, target, probe, output, *, motion_preference='standard', screenshots=False, asset_root=None):
    ...
    from .narration_assets import verified_asset_bytes
    _, asset_receipt = verified_asset_bytes(plan_frame_runtime(original, target), asset_root)
    from .visual_assets import verified_visual_bytes
    visual_bytes, visual_receipt = verified_visual_bytes(original, asset_root)
    ...
    for candidate in candidates(original, policy):
        ...
        try:
            ...
            if compiled.receipt.source_gate_passed:
                ...
                measure_kwargs={'screenshots':screenshots}
                if visual_bytes: measure_kwargs['asset_bytes']=visual_bytes
                measured=probe.measure(compiled,target,folder,**measure_kwargs)
    ...
    result={'schema_version':'bie.layout-repair-run.v1','status':status,
            ...
            'runtime_block':runtime_block,'asset_verification':asset_receipt,'source_published':False,'real_react':False,'real_remotion':False,
    ...
    if visual_bytes: result['visual_asset_verification']=visual_receipt
...
def repair_scene_layout(payload: dict, policy: dict | None, evidence_directory, *, target=None,
                        browser='/usr/bin/chromium', motion_preference='standard', screenshots=False, asset_root=None) -> dict:
    ...
    try:
        with ChromiumLayoutProbe(browser) as probe:
            result,_=_run_candidates(payload,policy,target,probe,output,
                                      motion_preference=motion_preference,screenshots=screenshots,asset_root=asset_root)
        return result
...
def repair_and_publish(payload, policy, destination, evidence_directory, *, target=None,
                       browser='/usr/bin/chromium', motion_preference='standard', screenshots=False, asset_root=None):
    ...
    receipt=publish_h3_scene(effective,destination,target=target,motion_preference=motion_preference,asset_root=asset_root)
...
def verify_repaired_workspace(root: Path) -> dict:
    ...
    from .frame_runtime_contract import plan_frame_runtime
    from .narration_assets import verified_asset_bytes
    _, audio_receipt = verified_asset_bytes(plan_frame_runtime(compiled.effective_document,CompilerQATarget(**envelope['target'])), Path(root)/'public')
    from .visual_assets import verified_visual_bytes
    visual_bytes, visual_receipt = verified_visual_bytes(compiled.effective_document,Path(root)/'public')
    if visual_bytes and result.get('visual_asset_verification')!=visual_receipt:
        raise CompilerQAError('REPAIR_VISUAL_ASSET_IDENTITY_CHANGED')
    if result.get('asset_verification', audio_receipt) != audio_receipt:
        raise CompilerQAError('REPAIR_AUDIO_IDENTITY_CHANGED')
