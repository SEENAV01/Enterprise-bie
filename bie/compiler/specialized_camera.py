...
def progress_source(c):
    discrete=c.parameters.get('_bie_h7_discrete_progress')
    if discrete is not None:
        return f'''if (!Number.isInteger(frame) || frame<0 || fps!=={discrete['fps']}) throw new Error("H7_VARIANT_FRAME_FPS");
  const milestones = {jsx(discrete['milestones'])};
  const progress = [...milestones].reverse().find(m=>frame>=m.frame)?.progress ?? 0;'''
