...
from .operational_math import operational_typeset_latex as isolated_typeset_latex
...
def compile_h3_scene(payload,*,target=None,motion_preference='standard'):
    ...
    media_presentations(effective, target)  # All visual assets must be source-bound before publication.
    layout=evaluate_frame_layout(effective,target)
    host=collect_host_toolchain()
    # Compile-local content cache: avoid rerunning the same isolated typeset work
    # for emission and independent byte-comparison QA. Never crosses host/input identity.
    from copy import deepcopy
    math_cache = {}
    def typesetter(expression, *, element_id, font_size=32):
        key = canonical_json({'expression':expression,'element_id':element_id,'font_size':font_size})
        if key not in math_cache:
            math_cache[key] = isolated_typeset_latex(expression, element_id=element_id,
                                      font_size=font_size, expected_host=host)
        return deepcopy(math_cache[key])
...
def publish_h3_scene(payload,destination,*,target=None,motion_preference='standard',asset_root=None):
    target=target or replace(CompilerQATarget(),compiler_version=VERSION)
    ...
    from .narration_assets import verified_asset_bytes, stage_verified_assets
    runtime_plan = plan_frame_runtime(result.effective_document, target)
    audio_bytes, _ = verified_asset_bytes(runtime_plan, asset_root)
    from .visual_assets import verified_visual_bytes
    media_bytes, _ = verified_visual_bytes(result.effective_document, asset_root)
    ...
    try:
        ...
        stage_verified_assets(audio_bytes, staging)
        stage_verified_assets(media_bytes, staging)
...
def require_h3_workspace(root,*,expected_scene_fingerprint=None,render_request=None):
    ...
    runtime_plan = plan_frame_runtime(result.effective_document, target)
    audio_bytes, _ = verified_asset_bytes(runtime_plan, root/'public')
    from .visual_assets import verified_visual_bytes
    media_bytes, _ = verified_visual_bytes(result.effective_document, root/'public')
    audio_paths = {'public/'+p for p in audio_bytes} | {'public/'+p for p in media_bytes}
    ...
    for p in root.rglob('*'):
        ...
        if rel.parts[0] in {'render-evidence','out','validation-runs'} and p.is_file():
            ...
            continue
        if p.is_file() and rel.as_posix() not in expected and rel.as_posix() not in extras and rel.as_posix() not in audio_paths:
