...
def emit_bound_layer(plan, element, *, mode="all"):
    ...
    if not bs and not caption:
        return None
    if mode not in {'all', 'content', 'controls'}:
        fail('FRAME_RUNTIME_LAYER_MODE', 'unknown generated runtime layer mode')
    props = {b['property_name'] for b in bs}
    if mode == 'content' and 'text' not in props and not caption:
        return None
    if mode == 'controls' and not props.intersection({'visible', 'opacity'}):
        return None
    replace_text = mode != 'controls' and ('text' in props or caption)
    prefix = {'all':'RuntimeLayer', 'content':'RuntimeContent', 'controls':'RuntimeControls'}[mode]
    name = component_name(prefix, eid)
    inner = '{children}'
    if replace_text:
...
'''
    if mode != 'all':
        source = source.replace('data-bie-runtime-target=', 'data-bie-runtime-mode="' + mode + '" data-bie-runtime-target=', 1)
