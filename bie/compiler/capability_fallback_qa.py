...
from .qa_common import diagnostic_message, CompilerQAError, QAFinding, digest, ordered_findings, token
...
def inspect_emitted_semantics(document, results: Iterable, *, typesetter=None, frozen_simulation_frames=None, target=None) -> tuple[QAFinding, ...]:
    ...
    for e in elements:
        ...
        hardened = {"shape":compile_shape_element,"diagram":compile_diagram_element,"highlight":compile_highlight_element,"text": compile_text_element, "annotation": compile_annotation_callout_element,
                    ...
                    "equation": compile_equation_element, "simulation": compile_simulation_element, "map": compile_map_element}
        hardened_verified = False
        if kind in hardened:
            try:
                if kind == "highlight":
                    canonical = hardened[kind](e,scene=document.to_dict(),target=target)
                elif kind == "equation":
                    canonical = hardened[kind](e, typesetter=typesetter)
                elif kind == "simulation":
                    canonical = hardened[kind](e, freeze_frame=(frozen_simulation_frames or {}).get(eid))
                else:
                    canonical = hardened[kind](e)
                hardened_verified = canonical == result
                if canonical != result:
                ...
                add(code if re.fullmatch(r"[A-Z][A-Z0-9_]+", code) else "ELEMENT_COMPILE_REJECTED", diagnostic_message(exc), eid)
        warnings_seen = set()
        if kind == "chart":
        ...
        if kind == "map" and not hardened_verified and "x * 1000" in result.source_text:
...
def evaluate_capability_fallbacks(document, registry, element_results: Iterable, *,
                                  ...
                                  approvals: Iterable[FallbackApproval] = (),
                                  typesetter=None, frozen_simulation_frames=None, target=None) -> CapabilityFallbackQAReceipt:
    token(profile, "profile")
    raw_doc = document.to_dict()
    require_sha256(document.fingerprint)
    results = tuple(element_results)
    findings = list(inspect_emitted_semantics(document, results, typesetter=typesetter, frozen_simulation_frames=frozen_simulation_frames, target=target))
    ...
    try:
        ...
        add("DSL_CAPABILITY_RESOLUTION_FAILED", diagnostic_message(exc))
