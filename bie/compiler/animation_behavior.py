...
def motion_contract(track) -> MotionContract:
    ...
    from .registered_actions import ACTIONS as REGISTERED_ACTIONS, registered_contract
    if action in REGISTERED_ACTIONS:
        return registered_contract(track)
    if params.get("schema_version") == "bie.comp-specialized-motion.v1":
        from .specialized_motion import specialized_contract
        return specialized_contract(track)
...
def frame_window(c: MotionContract, fps: int):
    ...
    if c.action == "morph" and c.parameters.get("schema_version") == "bie.comp-specialized-motion.v1" and end-start < 4*(len(c.parameters["states"])-1):
        fail("EQUATION_STATES_UNSAMPLED", "at least four intervals per state transition required")
    return start, end
...
def motion_state(c: MotionContract, frame: int, fps: int):
    ...
    if c.parameters.get("schema_version") == "bie.comp-registered-action.v1":
        from .registered_actions import registered_motion_state
        return registered_motion_state(c, frame, fps)
    if c.parameters.get("schema_version") == "bie.comp-specialized-motion.v1":
        from .specialized_motion import camera_state
        if c.action == "camera": return camera_state(c, frame, fps)
        frame_window(c, fps)
        return {}  # Content replacement stays inside its layer; ink QA is separate.
