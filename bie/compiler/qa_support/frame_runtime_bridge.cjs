// H6 DIAGNOSTIC ONLY: real TypeScript/JS execution, explicitly NOT React/Remotion.
...
const files=req.files,cache={}; let localFrame=0;const SEQ=Symbol('sequence'),AUDIO=Symbol('audio'),FRAG=Symbol('fragment');
...
const remotion={Interactive:{Div:'div'},AbsoluteFill:fill,Sequence:SEQ,
...
function load(id){
 ...
 const scope={exports:{},Math:safeMath,require:name=>{
   ...
   if(name==='@remotion/media')return {Audio:AUDIO};
...
const main=load('src/Scene.tsx');const runtime=load('src/runtime/frame-runtime.ts');
...
const result=JSON.stringify({execution_kind:'REAL_TS_WHOLE_SCENE_WITH_EXPLICIT_REACT_REMOTION_TEST_DOUBLES',
 runtime_bridge:'H6_LAZY_SEQUENCE_API_DOUBLE_NO_AUDIO_PLAYBACK',typescript_version:ts.version,trees,real_react:false,real_remotion:false,audio_played:false,accepted:false});
