options => {
 ...
 for(const layer of document.querySelectorAll('[data-bie-layer-id]')){
  ...
  const mediaOwner=layer.querySelector('[data-bie-media-viewport]');
  const owner=runtimeOwner||mediaOwner||(tracks.length?tracks[tracks.length-1]:layer);
  ...
  for(const el of descendants){
   ...
   if((el===owner||!el.hasAttribute('data-bie-track-id'))&&el.namespaceURI!=='http://www.w3.org/2000/svg'&&!el.hasAttribute('data-bie-media-viewport')&&el.clientWidth>0&&el.clientHeight>0&&(el.scrollWidth>el.clientWidth+1||el.scrollHeight>el.clientHeight+1))overflow=true;
   if(el.matches('img,video,canvas')&&mediaOwner){const b=rect(mediaOwner);if(b[2]>0&&b[3]>0)ink.push(b);}
  ...
  rows.push({element_id:id,frame:options.frame,visible:visible(owner)&&(!mediaOwner||visible(mediaOwner)),layer_box:rect(owner),ink_boxes:ink,text_boxes:texts,rendered_text:rendered,scroll_overflow:overflow,equation_em_px:em});
 }
 return rows;
}
