...
def compile_simulation_element(element, *, freeze_frame=None):
    eid, props, acc, src, rsn = require_type(element, "simulation")
    c = simulation_contract(props)
    if freeze_frame is not None and (type(freeze_frame) is not int or not 0 <= freeze_frame <= 36000):
        raise ElementCompilerError("REDUCED_SIMULATION_FRAME_INVALID: bounded integer frame required")
    frame_source = "useCurrentFrame()" if freeze_frame is None else str(freeze_frame)
...
export const {comp}: React.FC = () => {{
  const frame = {frame_source}; const {{fps}} = useVideoConfig();
