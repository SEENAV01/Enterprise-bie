...
def plan_frame_runtime(raw, target):
    ...
    if cfg is None:
        if 'compiler_h10' in raw.get('metadata', {}):
            fail('STATE_MOTION_RUNTIME_REQUIRED', 'composition needs the H6 frame runtime')
        return None
    ...
    from .state_motion_composition import validate_state_composition
    compositions = validate_state_composition(raw, bindings)
    ...
    result = {'schema_version': SCHEMA, 'input_identity': digest(raw), 'fps': fps, 'frame_count': frames,
              ...
              'accepted': False}
    if compositions:
        result['state_motion_compositions'] = compositions
