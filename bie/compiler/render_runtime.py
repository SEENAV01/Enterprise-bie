    ...
    with RenderEventLog(attempt / "events.jsonl", request.run_id, secrets=secrets) as log:
        ...
        try:
            ...
            if runner is None:
                ...
                from .real_paint import isolated_typecheck
                typecheck = isolated_typecheck(root,node=_tool(request.node_bin, 'Node'),evidence_directory=attempt/'isolated-typecheck')
            ...
            if runner is None:
                if tools['browser'] is None:
                    raise RenderFailure('EXPLICIT_BROWSER_REQUIRED','Pinned browser executable must be explicit; no download fallback')
                if request.concurrency > 4:
                    raise RenderFailure('RENDER_CONCURRENCY_POLICY','This operational profile permits at most four renderer workers')
                from .installed_toolchain import collect_installed_toolchain, require_same_toolchain
                from .real_paint import produce_actual_paint, require_actual_witness
                from .qa_scene_compile import CompilerQATarget
                installed_identity=collect_installed_toolchain(root,node=tools['node'],browser=tools['browser'])
                _write_json(attempt/'installed-toolchain.json',installed_identity)
                envelope=json.loads((root/'CHECKED_SCENE.json').read_text())
                try:
                    witness=produce_actual_paint(root,attempt/'actual-paint',node=tools['node'],browser=tools['browser'],target=CompilerQATarget(**envelope['target']))
                    require_actual_witness(witness,checked.manifest_sha256)
                except (ValueError,OSError,TypeError,KeyError) as exc:
                    raise RenderFailure('ACTUAL_PAINT_QA_BLOCKED',str(exc)) from exc
                _write_json(attempt/'actual-paint-witness.json',{'source_manifest_sha256':witness.source_manifest_sha256,'evidence_sha256':witness.evidence_sha256,'passed':witness.passed,'accepted':False})
            ...
            if installed_identity is not None:
                require_same_toolchain(installed_identity,collect_installed_toolchain(root,node=tools['node'],browser=tools['browser']))
        ...
        terminal_status = "PASS" if passed else ("CANCELLED" if failure_code == "CANCELLED" else
                         "BLOCKED" if failure_code in {"DEPENDENCIES_UNAVAILABLE", "TOOL_UNAVAILABLE", "SOURCE_QA_BLOCKED", "FULL_TYPECHECK_BLOCKED", "ACTUAL_PAINT_QA_BLOCKED", "H3_SOURCE_QA_BLOCKED", "EXPLICIT_BROWSER_REQUIRED"} else "FAIL")
