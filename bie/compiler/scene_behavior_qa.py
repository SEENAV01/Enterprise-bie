...
def validate_scene_behaviors(document, target):
    ...
    for i, t in enumerate(raw["tracks"]):
        ...
        try:
            ...
            from .registered_actions import is_registered, validate_registered_binding
            if is_registered(t):
                e=next(e for e in raw["elements"] if e["element_id"]==t["element_id"])
                contracts.setdefault("registered",[]).append(validate_registered_binding(t,e,target,document.duration_ms))
            if b >= frames:
    ...
    for i,e in enumerate(raw["elements"]):
        ...
        if e["element_type"] == "simulation":
            try:
                ...
                registered_clock=any(t["element_id"]==e["element_id"] and t["action"] in {"simulation_state","state_snapshots"} for t in raw["tracks"])
                if not registered_clock and end_seconds > (frames-1)/target.fps + 1e-12:
                ...
                if not registered_clock and omega/(2*3.141592653589793)*8 > target.fps:
