...
class ChromiumLayoutProbe:
    ...
    def measure(self, result, target, output: Path, *, screenshots: bool = False) -> dict:
        ...
        errors = []; observations = []; screenshot_records = []; network = []
        ...
        tools = {'browser_executable_sha256':file_identity(self.executable), 'browser_version':self.browser.version,
                 ...
                 'bridge_sha256':file_identity(SUPPORT/'layout_bridge.cjs'),
                 ...
                 'browser_os_sandbox_enabled':False,'network_request_policy':'ABORT_ALL',
                 'real_react':False,'real_remotion':False}
        ...
        try:
            for start in range(0,n,60):
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    child=run_bounded_process(['node',str(SUPPORT/'layout_bridge.cjs'),str(path)],cwd=td,timeout_s=60,max_output_bytes=64*1024*1024)
