...
import {applySemanticState,studioLevels,runtimeExperience} from "./react-runtime";
...
export function createRuntimeController(levelId:string){let disposed=false,bound=false,telemetrySequence=0;const disposers:(()=>void)[]=[];function on(target:HTMLElement,type:string,listener:any){target.addEventListener(type,listener);disposers.push(()=>target.removeEventListener(type,listener));}function dispose(){if(disposed)return;disposed=true;for(const remove of disposers)remove();disposers.length=0;for(const audio of playingAudio)audio.pause();playingAudio.clear();activeNarration=null;}let state=levelState(levelId),score=0,attempts=0,lastFeedback="Ready";const completedChallenges=new Set<string>();validateState(levelId,state);const telemetry:Record<string,unknown>[]=[];const actions=(interactionProgram.actions_and_events as readonly any[]).filter((x:any)=>x.level_id===levelId&&x.action_id);
 ...
 const challenge=challengeFunctions[JSON.stringify([levelId,challengeId])];if(!challenge)throw new Error("GAME_RUNTIME_CHALLENGE_MISSING");
 const nextState=applyEffects(levelId,state,meta.effects);const failed=challenge.failure(nextState);const succeeded=!failed&&challenge.success(nextState);
 state=nextState;attempts++;const gameId=(stateMachine.games as readonly any[]).find((g:any)=>g.levels.some((l:any)=>l.level_id===levelId))?.game_id;const scoring=(scoringProgram.games as any)[gameId];if(scoring){const points=succeeded&&!completedChallenges.has(challengeId)?Number(scoring.correct_points):failed?Number(scoring.incorrect_points):0;score=Math.max(Number(scoring.floor),score+points);}if(succeeded)completedChallenges.add(challengeId);const fb=(feedbackProgram.feedback as readonly any[]).find((x:any)=>x.challenge_id===challengeId&&x.level_id===levelId);lastFeedback=succeeded?(fb?.success||"Correct"):failed?(fb?.failure||"Not yet"):"In progress";const fired:string[]=[];for(const a of adaptationMetadata.rules as readonly any[]){if(a.level_id!==levelId)continue;const fn=adaptationFunctions[a.function_name];if(fn&&fn(state))fired.push(a.action+":"+(a.target_id||""));}const event=sanitizeTelemetry({event:"mechanic_completed",game_id:gameId,level_id:levelId,challenge_id:challengeId,attempt_number:attempts,outcome_code:succeeded?"succeeded":failed?"failed":"in_progress",mechanic_id:action.mechanic_id,objective_id:(action.objective_ids||[])[(action.challenge_ids||[]).indexOf(challengeId)]||"",adaptation_id:fired[0]||""});if((telemetryProgram.events as readonly string[]).includes("mechanic_completed")){telemetry.push(event);const cfg=window.__BIE_GAME_TELEMETRY_CONFIG__;if(cfg?.enabled&&window.__BIE_GAME_TELEMETRY_SINK__){telemetrySequence++;window.__BIE_GAME_TELEMETRY_SINK__({...event,session_id:cfg.session_id,sequence_id:telemetrySequence,policy_id:cfg.policy_id});}}playCue(levelId,"mechanic_completed");applySemanticState(state,levelId);const out=document.getElementById("bie-game-feedback");if(out)out.textContent=lastFeedback;return{applied:true,action_id:actionId,rule_id:action.rule_id,mechanic_id:action.mechanic_id,state:cloneState(state),score,feedback:lastFeedback,adaptations:fired,telemetry:event};}
 ...
 return Object.freeze({dispatch,bind,dispose,playNarration:()=>playCue(levelId,"level_start"),getState:()=>cloneState(state),getScore:()=>score,getFeedback:()=>lastFeedback,getTelemetry:()=>telemetry.slice(),actionIds:()=>actions.map((x:any)=>x.action_id),audioRuntimeAvailable:true,reducedMotionSupported:true});}
'''
