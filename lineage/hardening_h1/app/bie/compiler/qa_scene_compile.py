...
class CompilerQATarget:
    ...
    compiler_version: str = "1.1.0-comp-h1"
    seed: int = 0
...
def native_qa_capabilities() -> CapabilityRegistry:
    ...
    for kind in sorted(EMITTERS):
        registry.register(CompilerCapability("comp:" + kind, "1.1.0", (kind,), ("render",), ("web",), True))
    return registry
...
def compile_scene_for_qa(payload, *, target: CompilerQATarget | None = None,
    ...
    for e in doc.elements:
        ...
        try:
            ...
            findings.append(QAFinding(code, "ERROR", str(exc), f"$.elements[{e.element_id}]"))
