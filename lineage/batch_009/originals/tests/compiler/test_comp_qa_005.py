...
class MultidomainBenchmarkTests(unittest.TestCase):
    ...
    def test_ten_positive_source_cases_pass(self):self.assertEqual(self.receipt.positive_source_passes,10)
    def test_nine_expected_defects_are_not_positive_outputs(self):
        self.assertEqual(self.receipt.negative_cases_detected,9)
    ...
    def test_expected_negative_keeps_mapped_executable_diagnostic(self):
        row=next(r for r in self.receipt.cases if r.case_id=='reject-jsx-braces');self.assertGreater(row.diagnostic_mapping_count,0)
