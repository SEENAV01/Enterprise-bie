...
class CompilerQAIntegrationTests(unittest.TestCase):
    ...
    def test_actual_source_diagnostic_maps_to_negative_fixture(self):
        ...
        self.assertFalse(r.passed);self.assertTrue(all(d.origin.element_id=='e0' for d in r.diagnostics))
    def test_static_source_is_not_assumed_animated(self):
        b=bundle('reject-metadata-animation')
        self.assertTrue(b.animation_results);self.assertFalse(b.source_contract_passed)
        self.assertIn('ANIMATION_TRACK_NO_VISUAL_EFFECT',{f.code for f in b.findings})
