...
def compile_adaptation(ctx:CompilerContext):
    ...
    for exp in ctx.document.experiences:
      for level in exp.levels:
        ...
        for r in rules:
            fn=ts_identifier('adapt',r.adaptation_id);expr=compile_expr(r.condition,tmap);defs.append(f'export function {fn}(state: RuntimeState): boolean {{ return Boolean({expr}); }}')
