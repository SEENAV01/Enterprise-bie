...
def resolve_reduced_motion(raw, target, preference='standard'):
    ...
    if preference == 'reduced' and any(is_specialized(t) for t in raw.get('tracks', [])):
        fail('SPECIALIZED_REDUCED_VARIANT_REQUIRED', 'A fade cannot replace equation steps or a camera/trace teaching sequence; a reviewed semantic alternative is required.')
    ...
    for e in raw['elements']:
        ...
        tracks=[t for t in raw.get('tracks',[]) if t['element_id']==eid]
        ref=e.get('accessibility',{}).get('reduced_motion_variant')
        if ref:referenced.append(ref)
        needs=bool(tracks) or kind in {'simulation','particle_system','video'}
