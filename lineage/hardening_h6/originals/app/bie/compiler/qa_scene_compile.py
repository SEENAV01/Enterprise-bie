...
def native_qa_capabilities() -> CapabilityRegistry:
    ...
    for kind in sorted(EMITTERS):
        registry.register(CompilerCapability("comp:" + kind, "1.2.0", (kind,), ("render",), ("web",), True))
    return registry
...
def compile_scene_for_qa(payload, *, target: CompilerQATarget | None = None,
    ...
    for e in doc.elements:
        ...
        for track in tracks:
            try:
                ...
                result_track = (compile_animation_track(track, element=element, typesetter=typesetter)
                                if is_specialized(track) else compile_animation_track(track))
        ...
        content_names = {r.source_path.rsplit("/", 1)[-1][:-4] for t, r in zip(tracks, animation_results[-len(tracks):])
                         if is_specialized(t) and t.action in {"morph", "trace"}} if tracks else set()
        ...
        if runtime_plan is not None:
            bound = emit_bound_layer(runtime_plan, element)
    ...
    capability = evaluate_capability_fallbacks(doc, native, element_results, typesetter=typesetter, frozen_simulation_frames=frozen_simulation_frames)
