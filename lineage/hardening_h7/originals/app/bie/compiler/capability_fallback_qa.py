...
def inspect_emitted_semantics(document, results: Iterable, *, typesetter=None, frozen_simulation_frames=None) -> tuple[QAFinding, ...]:
...
def evaluate_capability_fallbacks(document, registry, element_results: Iterable, *,
                                  ...
                                  typesetter=None, frozen_simulation_frames=None) -> CapabilityFallbackQAReceipt:
    token(profile, "profile")
    raw_doc = document.to_dict()
    require_sha256(document.fingerprint)
    results = tuple(element_results)
    findings = list(inspect_emitted_semantics(document, results, typesetter=typesetter, frozen_simulation_frames=frozen_simulation_frames))
