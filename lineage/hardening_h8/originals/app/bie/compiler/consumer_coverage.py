...
def inspect_consumer_coverage() -> dict:
    ...
    from . import qa_scene_compile, animation_behavior, specialized_motion
    ...
    def identity(path):
        ...
        return name
    for mod in (registry, qa_scene_compile, animation_behavior, specialized_motion): identity(mod.__file__)
    rows = []
    for kind in kinds:
    ...
    for action in actions:
        dispatcher = animation_behavior.motion_contract if action in generic else specialized_motion.specialized_contract if action in special else None
    ...
    extra = {'elements': sorted(set(qa_scene_compile.EMITTERS) - set(kinds)), 'actions': sorted((generic | special) - set(actions))}
