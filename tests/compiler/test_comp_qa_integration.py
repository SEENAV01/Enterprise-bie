...
class CompilerQAIntegrationTests(unittest.TestCase):
    ...
    def test_invalid_shape_contract_is_failure_not_text_fallback(self):
        raw=case_raw()['document'];raw.pop('fingerprint');raw['elements'][0]['element_type']='shape';raw['elements'][0]['props']={'kind':'circle'}
        result=compile_scene_for_qa(raw)
        self.assertFalse(result.source_contract_passed)
        self.assertIn('PRIMITIVE_FIELDS_INVALID',{f.code for f in result.findings})
