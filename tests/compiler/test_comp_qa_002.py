...
from tests.compiler.qa_test_support import document, bundle, historical_semantics
...
class CapabilityFallbackQATests(unittest.TestCase):
    ...
    def test_nonbar_is_not_safe_fallback(self):self.assertIn('CHART_KIND_DOWNGRADE',{f.code for f in historical_semantics('reject-chart-kind')})
    def test_negative_chart_sign_cannot_disappear(self):self.assertIn('CHART_SIGN_LOSS',{f.code for f in historical_semantics('reject-chart-sign')})
    def test_vector_third_dimension_cannot_disappear(self):self.assertIn('VECTOR_Z_COMPONENT_DROPPED',{f.code for f in historical_semantics('reject-vector-z')})
    def test_raw_equation_not_typeset_equation(self):self.assertIn('EQUATION_TYPESETTING_NOT_IMPLEMENTED',{f.code for f in historical_semantics('reject-untypeset-equation')})
    def test_raw_text_braces_are_executable(self):self.assertIn('TEXT_LITERAL_JSX_INJECTION',{f.code for f in historical_semantics('reject-jsx-braces')})
    def test_state_dump_not_simulation(self):self.assertIn('SIMULATION_STATE_ONLY',{f.code for f in historical_semantics('reject-state-only-simulation')})
    def test_crs_projection_not_assumed(self):self.assertIn('MAP_PROJECTION_UNVERIFIED',{f.code for f in historical_semantics('reject-geographic-crs')})
    def test_invalid_model_index_blocked(self):self.assertIn('MODEL2D_EDGE_INDEX_INVALID',{f.code for f in historical_semantics('reject-model-edge')})
