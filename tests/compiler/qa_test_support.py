...
# H9 context-only migration; original H2 corpus and golden source hashes remain immutable.
FIXTURES = ROOT / 'fixtures/comp_h9/legacy_qa_compat'
...
def historical_semantics(cid):
    ...
    data=json.loads((ROOT/'fixtures/comp_h1/historical_emitter_results.json').read_text())
    rows=[]
    for row in data[cid]:
