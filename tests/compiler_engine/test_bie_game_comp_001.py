...
from bie.game_engine.compiler_engine.game_ir import compile_game_ir as FUNC
...
class Comp001Tests(unittest.TestCase):
    def test_valid_compile(self):
        a=FUNC(compiler_context());self.assertEqual(a.kind,ArtifactKind.GAME_IR);self.assertIn('game-ir.json',a.path);a.validate()
