...
def env(clock=None):
    scheduler=CapabilityScheduler([
        WorkerCapabilities("w1",{"python","sandbox"},8,16,max_concurrency=2)
    ])
    ...
    req=StageResourceRequirement("REASONING",{"python"},1,1,slot_cost=1)
...
class ExecutorRuntimeTests(unittest.TestCase):
    ...
    def test_scheduler_failure_means_no_execution(self):
        scheduler=CapabilityScheduler([WorkerCapabilities("w1",{"node"},4,8,max_concurrency=1)])
