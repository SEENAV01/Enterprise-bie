...
ctx=sample_context
from bie.game_engine.mechanics_engine import resource_tradeoff
def valid():return resource_tradeoff.execute(ctx(),{'resources':{'time':10,'points':20}}, {'time':-2,'points':5},{'time':(0,10),'points':(0,100)})
def invalids():return [(lambda:resource_tradeoff.execute(ctx(),{'resources':{'time':1}}, {'time':-2},{'time':(0,10)})),(lambda:resource_tradeoff.execute(ctx(),{'resources':{'time':1}}, {'time':0},{'time':(0,10)}))]
from bie.game_engine.mechanics_engine import resource_tradeoff as MODULE
