...
ctx=sample_context
from bie.game_engine.mechanics_engine import drag_drop
def valid():return drag_drop.execute(ctx(),{'position':(0,0),'placed':False},(0.1,0.1),.25)
def invalids():return [(lambda:drag_drop.execute(ctx(),{'position':(0,0),'placed':False},(5,5),.25)),(lambda:drag_drop.execute(ctx(),{'position':'x'},(0,0),.25))]
from bie.game_engine.mechanics_engine import drag_drop as MODULE
