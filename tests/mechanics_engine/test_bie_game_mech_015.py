...
ctx=sample_context
from bie.game_engine.mechanics_engine import branching_scenario
def valid():return branching_scenario.execute(ctx(),{}, {'start':{'yes':'terminal:success','no':'retry'},'retry':{'again':'start'}},'start','yes')
def invalids():return [(lambda:branching_scenario.execute(ctx(),{}, {'start':{'yes':'missing'}},'start','yes')),(lambda:branching_scenario.execute(ctx(),{}, {'start':{'yes':'terminal:x'}},'start','no'))]
from bie.game_engine.mechanics_engine import branching_scenario as MODULE
