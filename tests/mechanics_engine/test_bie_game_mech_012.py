...
ctx=sample_context
from bie.game_engine.mechanics_engine import prediction_observe
def valid():return prediction_observe.execute(ctx(),{'observation_revealed':False},2,2.5,True)
def invalids():return [(lambda:prediction_observe.execute(ctx(),{'observation_revealed':False},2,3,False)),(lambda:prediction_observe.execute(ctx(),{'observation_revealed':True},2,3,True))]
from bie.game_engine.mechanics_engine import prediction_observe as MODULE
