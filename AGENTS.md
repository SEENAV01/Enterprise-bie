...
Current GAME Section 15 source and adoption provenance are in `bie/game_engine/`, `manifests/game_section15_adoption.json` and `docs/evidence/game-section15/`. Preserve the canonical legacy contract, original archives and amendment before-images. The required integrated gate includes all 1,123 GAME tests and all earlier enterprise tests. Use the approved CI supervisor environment for GAME browser isolation; never weaken its sandbox or reuse isolated-candidate success as canonical success. Follow Sections 16, 17, 18 before Task 028 and retain product acceptance blockers.
