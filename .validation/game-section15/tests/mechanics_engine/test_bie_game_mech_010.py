...
ctx=sample_context
from bie.game_engine.mechanics_engine import map_interaction
def valid():return map_interaction.execute(ctx(),{}, {'A':{'coord':(28.6,77.2),'source_ref':'source:a'},'B':{'coord':(28.45,77.03),'source_ref':'source:b'}},'A','B')
def invalids():return [(lambda:map_interaction.execute(ctx(),{}, {'A':{'coord':(200,0),'source_ref':'s'},'B':{'coord':(0,0),'source_ref':'s2'}},'A','B')),(lambda:map_interaction.execute(ctx(),{}, {'A':{'coord':(0,0)},'B':{'coord':(1,1),'source_ref':'s'}},'A','B'))]
from bie.game_engine.mechanics_engine import map_interaction as MODULE
