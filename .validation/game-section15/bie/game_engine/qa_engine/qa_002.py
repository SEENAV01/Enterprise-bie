...
def evaluate(document,bundle):
    document.validate();rules=[];state_ids={};source={};refs=[]
    for exp in document.experiences:
      for level in exp.levels:
        state_ids[(exp.game_id,level.level_id)]=set(level.state.type_map());rules.extend(level.interaction.rules)
        source.update({(exp.game_id,level.level_id,r.rule_id):r for r in level.interaction.rules})
    ...
    data=_metadata(art.content);compiled={(x.get('game_id'),x.get('level_id'),x['rule_id']):x for x in data['rules']}
    if len(compiled)!=len(data['rules']):findings.append(finding('BIE-GAME-QA-002',6,'RULE_DUPLICATE_SCOPE','duplicate game/level/rule identity',refs=art.source_refs))
    for key in set(source)&set(compiled):
        expected=[{'target':e.target_variable_id,'kind':e.kind.value,'value':e.value} for e in source[key].effects]
        if compiled[key].get('effects')!=expected:findings.append(finding('BIE-GAME-QA-002',7,'RULE_EFFECT_MISMATCH',str(key),refs=art.source_refs))
    ...
    if missing:findings.append(finding('BIE-GAME-QA-002',1,'RULES_MISSING',str(missing),refs=art.source_refs))
    if extra:findings.append(finding('BIE-GAME-QA-002',2,'RULES_EXTRA',str(extra),refs=art.source_refs))
    ...
    for key,r in source.items():
      for e in r.effects:
        if e.target_variable_id not in state_ids[key[:2]]:target_bad.append(r.rule_id+':'+e.target_variable_id)
