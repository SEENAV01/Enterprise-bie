...
def to_compiler_context(game:MaterializedGame,*,assets=None,mechanic_events=(),experience_profile=None):
    game.validate();return CompilerContext(game.document,game.text_catalog,game.scoring_policies,game.mastery_policies,dict(assets or {}),tuple(mechanic_events),game.runtime_capabilities,game.telemetry_allowlist,CompilerSecurityPolicy(), 'studio-enterprise-v1',False,experience_profile).validate()
