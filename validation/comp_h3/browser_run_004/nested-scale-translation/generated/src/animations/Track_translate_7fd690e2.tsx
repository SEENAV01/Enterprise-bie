...
export const evaluateTrackStyle = (frame: number, fps: number): React.CSSProperties => {
  ...
  return {translate: (0.0+(30.0-0.0)*progress) + "px " + 0 + "px"};
};
export const Track_translate_7fd690e2: React.FC<React.PropsWithChildren> = ({children}) => {
  const frame = useCurrentFrame(); const {fps} = useVideoConfig();
  const style = evaluateTrackStyle(frame, fps);
  return <div data-bie-track-id={"translate"} data-bie-element-id={"e0"} data-bie-action={"transform"}
