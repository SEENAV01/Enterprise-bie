...
export const evaluateTrackStyle = (frame: number, fps: number): React.CSSProperties => {
  ...
  const startFrame = Math.round((500 / 1000) * fps);
  const endFrame = Math.round((1500 / 1000) * fps) - 1;
...
export const Track_second_16367aac: React.FC<React.PropsWithChildren> = ({children}) => {
  const frame = useCurrentFrame(); const {fps} = useVideoConfig();
  const style = evaluateTrackStyle(frame, fps);
  return <div data-bie-track-id={"second"} data-bie-element-id={"e0"} data-bie-action={"enter"}
