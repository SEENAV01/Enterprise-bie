...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--output',type=Path,default=ROOT/'verification/dir_execution_tests.json')
    ...
    summary.update(test_files=len(records),passed=bool(records) and all(r['passed'] for r in records),python=platform.python_version(),
        original_dir_tests=sum(r['tests'] for r in records if not Path(r['file']).name.startswith('test_hard_') and Path(r['file']).name!='test_execution_integration.py'),
        previous_hardening_tests=sum(r['tests'] for r in records if Path(r['file']).name in ('test_hard_contracts_001.py','test_hard_semantic_001.py','test_hard_integration.py')),
        new_input_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_hard_inputs_001.py'),
        new_director_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_hard_director_001.py'),
        new_executor_integration_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_execution_integration.py'),
