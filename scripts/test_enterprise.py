"""Run every imported enterprise unit test against the combined app/bie tree.
...
# Original packages use both app.bie.* and older package/bare-module imports.
# All compatibility paths resolve to the combined source, never batch copies.
sys.path[:0]=[str(ROOT),str(SOURCE)]+[str(p) for p in sorted(SOURCE.iterdir()) if p.is_dir()]
...
def main():
    ...
    for path in paths:
        ...
        results.append(item)
    summary={'scope':'Imported enterprise unit tests against combined source; no production or real-book acceptance claimed',
