"""Run preserved DIR plus window construction, generation and integration gates."""
import argparse
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--output',type=Path,default=ROOT/'verification/dir_windows_tests.json')
    ...
    summary.update(test_files=len(records),passed=bool(records) and all(r['passed'] for r in records),python=platform.python_version(),
        original_dir_tests=sum(r['tests'] for r in records if not Path(r['file']).name.startswith('test_hard_') and Path(r['file']).name not in ('test_execution_integration.py','test_annotation_integration.py','test_repair_integration.py','test_context_integration.py','test_window_integration.py')),
