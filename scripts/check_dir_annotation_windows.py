"""Run preserved DIR plus annotation-window and global-review gates."""
import argparse
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--output',type=Path,default=ROOT/'verification/dir_annotation_windows_tests.json')
    ...
    summary.update(test_files=len(records),passed=bool(records) and all(r['passed'] for r in records),python=platform.python_version(),
        original_dir_tests=sum(r['tests'] for r in records if not Path(r['file']).name.startswith('test_hard_') and Path(r['file']).name not in ('test_execution_integration.py','test_annotation_integration.py','test_repair_integration.py','test_context_integration.py','test_window_integration.py','test_annotation_window_integration.py')),
        ...
        new_window_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_hard_windows_001.py'),
        new_window_directing_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_hard_long_directing_001.py'),
        new_window_integration_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_window_integration.py'),
        new_annotation_window_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_hard_annotation_windows_001.py'),
        new_global_review_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_hard_global_review_001.py'),
        new_annotation_window_integration_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_annotation_window_integration.py'),
