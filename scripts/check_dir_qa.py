...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--output',type=Path,default=ROOT/'verification/dir_qa_tests.json')
    ...
    summary.update(test_files=len(records),passed=bool(records) and all(r['passed'] for r in records),python=platform.python_version(),
        previous_dir_tests=sum(r['tests'] for r in records if not Path(r['file']).name.startswith('test_qa_')),
        qa_unit_tests=sum(r['tests'] for r in records if Path(r['file']).name in [f'test_qa_{i:03}.py' for i in range(1,8)]),
        qa_cross_contract_tests=sum(r['tests'] for r in records if Path(r['file']).name in ('test_qa_contracts.py', 'test_qa_exit_contracts.py')),
        duration_seconds=round(time.monotonic()-start,3))
    report={'executed_at_utc':datetime.now(timezone.utc).isoformat(),
        'scope':'Local DIR/TIME/SYNC/QA source and contract verification, not canonical integration or real-book/media acceptance',
