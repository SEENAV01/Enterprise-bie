...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--output',type=Path,default=ROOT/'verification/dir_hardening_tests.json')
    ...
    summary.update(test_files=len(records),passed=bool(records) and all(r['passed'] for r in records),python=platform.python_version(),
        original_dir_tests=sum(r['tests'] for r in records if not Path(r['file']).name.startswith('test_hard_')),
        hardening_unit_tests=sum(r['tests'] for r in records if Path(r['file']).name in ('test_hard_contracts_001.py','test_hard_semantic_001.py')),
        hardening_integration_tests=sum(r['tests'] for r in records if Path(r['file']).name=='test_hard_integration.py'),
